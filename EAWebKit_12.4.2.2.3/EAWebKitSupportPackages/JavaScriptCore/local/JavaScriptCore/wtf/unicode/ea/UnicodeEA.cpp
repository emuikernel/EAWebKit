...
namespace WTF {
	namespace Unicode {
		...
		UChar32 toLower(UChar32 c)
		{
            ...
            const JSText::Char source[2] = {static_cast<const JSText::Char>(c), 0};
            JSText::Char result[2]; // 1 + terminator;
            JSTextInterface* pTI = JSGetTextInterface();
            pTI->ConvertCase(&source[0], 1, &result[0], 2, JSText::kCaseTypeLower);
            return static_cast<UChar32>(result[0]); 
		}
		...
		UChar32 toUpper(UChar32 c)
		{
            ...
            JSText::Char result[2]; // 1 + terminator;
            JSTextInterface* pTI = JSGetTextInterface();
            pTI->ConvertCase(&source[0], 1, &result[0], 2, JSText::kCaseTypeUpper);
            return static_cast<UChar32>(result[0]); 
        }
