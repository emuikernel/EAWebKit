        ...
        // list of string, uri, counter, attr, i
    {
        ...
        for (CSSValueListIterator i = value; i.hasMore(); i.advance()) {
            ...
            if (item->isImageGeneratorValue()) {
				//+EAWebKitChange
				//10/19/2012 
				RefPtr<StyleImage> tempGeneratedImage(static_cast<CSSImageGeneratorValue*>(item)->generatedImage());
				m_style->setContent(tempGeneratedImage, didSet);
				//-EAWebKitChange
                didSet = true;
            }
            ...
            case CSSPrimitiveValue::CSS_STRING:
				{
					//+EAWebKitChange
					//10/19/2012
					WTF::String tempStringValue(contentValue->getStringValue().impl());
					m_style->setContent(tempStringValue, didSet);
					//-EAWebKitChange
					didSet = true;
					break;
				}
            case CSSPrimitiveValue::CSS_ATTR: {
				...
				//+EAWebKitChange
				//10/19/2012
				WTF::String tempStringValue(m_element->getAttribute(attr).impl());
                m_style->setContent(tempStringValue, didSet);
				//-EAWebKitChange
                didSet = true;
            ...
            case CSSPrimitiveValue::CSS_URI: {
				...
				//+EAWebKitChange
				//10/19/2012
				RefPtr<StyleImage> tempGeneratedImage(cachedOrPendingFromValue(CSSPropertyContent, static_cast<CSSImageValue*>(contentValue)));
                m_style->setContent(tempGeneratedImage, didSet);
				//-EAWebKitChange
				didSet = true;
                break;
...
//+EAWebKitChange
//10/17/2011 - Modifed to transfer shadow data to the font description.
#if PLATFORM(EA)            
            {
                const ShadowData *pShadowData = m_parentStyle->textShadow();
                if (pShadowData)
                {    
                    FontDescription fontDescription = m_style->fontDescription();       
                    fontDescription.setEffect(pShadowData->x(), pShadowData->y(),pShadowData->blur(),
                        pShadowData->color().rgb(), m_style->color().rgb());    
                    setFontDescription(fontDescription);
                    updateFont();
                }
