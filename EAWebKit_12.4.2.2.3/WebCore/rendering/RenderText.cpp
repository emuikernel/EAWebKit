...
//+EAWebKitChange
//5/30/2012
    // CSidhall: We need to initialize the string data buffer for cases where the startOfWord is > 1.
    // This can occur if the first word has a space before it and UChar previous is also a space (default if no previous string), so 2 spaces, 
    // leading to startOfWord index of 2 where buffer position 0 is never init. 
    if (startOfWord > 1)
    {
        for (int i = 0; i < (startOfWord - 1); i++)
            data[i] =  ' ';
    }
//-EAWebKitChange
